...
kcrontab_timer_thread::kcrontab_timer_thread(int max_fd_num):
	active_(true),
	ep_fd_(-1),
	max_fd_num_(max_fd_num)
{ }

kcrontab_timer_thread::~kcrontab_timer_thread()
{
}

int kcrontab_timer_thread::init()
{
...
int kcrontab_timer_thread::start()
{
	return this->timer_thread_.start(this);
}

int kcrontab_timer_thread::add_timer(s_timer *ptimer)
{
...
int kcrontab_timer_thread::delete_timer(int timer_id)
{
	struct epoll_event ev;
	ev.data.fd = timer_id;
	ev.events = EPOLLIN;
...
int kcrontab_timer_thread::delete_timer(s_timer *ptimer)
{
...
void kcrontab_timer_thread::stop_kcrontab()
{
	this->active_ = false;
}
